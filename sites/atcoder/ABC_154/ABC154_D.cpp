...
double sum_suretu(ll n) {
    // 初項 1, 公差 1, 項数 n の等差数列の和
    return ((double)n/2)*((double)(1+n));
}
...
int main() {
    ...
    // calculation
    vector<double> ex(N);
    rep(i, N) ex[i] = sum_suretu(vec[i])/vec[i];
    
    double partial_sum = 0;
    rep(i, K) partial_sum += ex[i];  // 部分和の初期値
    double res = partial_sum;
    
    for (ll i = 1; i < N-K+1; ++i) {
        partial_sum = partial_sum - ex[i-1] + ex[i+K-1];
        res = max(res, partial_sum);
    }
    cout << res << "\n";
